...
sbi_new = '<div class="marquee-item" style="cursor: pointer;" onclick="openBankOffer(\'SBIFLIGHT\', \'Get upto 15% off on flight booking\')"><span class="m-icon">🏦</span> <strong>SBI Bank:</strong> Get upto 15% off on flight booking</div>'
...
axis_new = '<div class="marquee-item" style="cursor: pointer;" onclick="openBankOffer(\'AXIS20\', \'Get upto 20% off on flight booking\')"><span class="m-icon">💳</span> <strong>AXIS BANK Debit card:</strong> Get upto 20% off on flight booking</div>'

mobi_old = '<div class="marquee-item"><span class="m-icon">📱</span> <strong>MobiKwik:</strong> Get upto 10% cashback</div>'
mobi_new = '<div class="marquee-item" style="cursor: pointer;" onclick="openBankOffer(\'MOBI10\', \'Get upto 10% cashback\')"><span class="m-icon">📱</span> <strong>MobiKwik:</strong> Get upto 10% cashback</div>'
...
if 'id="bankOfferDrawer"' not in html:
    html = html.replace('<!-- Haptic audio simulation nodes -->', bank_offer_drawer_html + '\n    <!-- Haptic audio simulation nodes -->')
...
if 'function openBankOffer' not in app_js:
    with open('app.js', 'a') as f:
        f.write(new_js)

print("Marquee interaction added!")
