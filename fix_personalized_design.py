...
# 2. Fix Rewards Banner (light blue background, white text, green text)
old_banner = """                                            <!-- Rewards & Savings Banner (Premium Glassmorphic) -->
...
new_banner = """                                            <!-- Rewards & Savings Banner (Premium Glassmorphic) -->
                                            <div style="margin: 12px 16px; background: rgba(56, 189, 248, 0.2); border-radius: 12px; padding: 12px; display: flex; flex-direction: column; gap: 8px; border: 1px solid rgba(255, 255, 255, 0.3); position: relative; overflow: hidden; box-shadow: inset 0 2px 10px rgba(255,255,255,0.2);">
                                                <div style="display: flex; align-items: center; justify-content: space-between; z-index: 1;">
                                                    <div style="display: flex; align-items: center; gap: 8px;">
                                                        <span style="background: linear-gradient(135deg, #f59e0b, #d97706); color: #fff; padding: 3px 8px; border-radius: 6px; font-size: 10px; font-weight: 900; letter-spacing: 0.5px; box-shadow: 0 2px 4px rgba(245, 158, 11, 0.4);">BLUCHIP</span>
                                                        <span style="font-size: 13px; font-weight: 800; color: #ffffff;">Earns 500 Points</span>
                                                    </div>
                                                </div>
                                                <div style="font-size: 12px; font-weight: 700; color: #4ade80; display: flex; align-items: center; gap: 6px; z-index: 1;">
                                                    <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1.41 16.09V20h-2.67v-1.93c-1.71-.36-3.16-1.46-3.27-3.4h1.96c.1 1.05.82 1.87 2.65 1.87 1.96 0 2.4-.98 2.4-1.59 0-.83-.44-1.61-2.67-2.14-2.48-.6-4.18-1.62-4.18-3.67 0-1.72 1.43-2.81 3.11-3.14V3.9h2.67v1.95c1.28.32 2.64 1.34 2.87 3.03h-1.95c-.2-1.04-.99-1.64-2.25-1.64-1.55 0-2.11.85-2.11 1.49 0 .91.56 1.45 2.83 2.01 2.8.68 4.02 1.87 4.02 3.73 0 2.06-1.56 3.06-3.41 3.42z"/></svg>
                                                    You save ₹500 on your usual add-ons!
                                                </div>
                                            </div>"""
...
# 3. Fix CTA Button size
old_btn = """                                                <div class="pass-book-cta" onclick="event.stopPropagation(); triggerHaptic('medium', 'Book CTA'); navigateTo('passenger')" style="background: linear-gradient(135deg, #fde047 0%, #f59e0b 100%); color: #451a03; padding: 10px 24px; border-radius: 24px; font-weight: 900; font-size: 15px; cursor: pointer; box-shadow: 0 6px 16px rgba(245, 158, 11, 0.4);">
                                                    Book at ₹4,999
                                                </div>"""

new_btn = """                                                <div class="pass-book-cta" onclick="event.stopPropagation(); triggerHaptic('medium', 'Book CTA'); navigateTo('passenger')" style="background: linear-gradient(135deg, #fde047 0%, #f59e0b 100%); color: #451a03; padding: 8px 16px; border-radius: 20px; font-weight: 800; font-size: 14px; cursor: pointer; box-shadow: 0 4px 10px rgba(0,0,0,0.15);">
                                                    Book at ₹4,999
                                                </div>"""
