...
function showGameWin() {
    ...
    if (carouselSlide) {
        ...
        init3DCurvedCoverflow('offersCarousel', 'offersDots', 280, 220); // Re-initialize to fix layout
    }
}
...
function navigateTo(screenName) {
    ...
    if (screenName === 'addons' && window.appState && window.appState.upfrontUnlocked) {
        const btn = document.getElementById('addonChooseSeatBtn');
        if (btn) btn.innerText = 'Free Seat';
    }
...
function init3DCurvedCoverflow(carouselId, dotsContainerId, cardWidth, cardHeight) {
    ...
    const allSlides = Array.from(carousel.querySelectorAll('.carousel-slide, .trending-slide-card'));
    const slides = allSlides.filter(slide => window.getComputedStyle(slide).display !== 'none');
    const N = slides.length;
    if (N === 0) return;

    // Dynamically regenerate dots to match the number of visible slides
...
function openDatePicker(mode = 'departure') {
    triggerHaptic('medium', 'Open date picker');
    
    pickerState.pickingMode = mode;
...
function updateDateDisplay() {
    ...
    if (pickerState.returnDate) {
        const r = pickerState.returnDate;
        const formattedRet = `${r.getDate()} ${months[r.getMonth()]}`;
        const retNode = document.getElementById('valReturnText');
        if (retNode) {
            retNode.innerText = formattedRet;
            retNode.style.color = '#0f172a';
            retNode.style.fontWeight = '700';
        }
    }
}
...
function renderCalendar() {
    ...
    for (let day = 1; day <= totalDays; day++) {
        ...
        if (cellDate < today) {
            ...
            cell.onclick = () => {
                if (pickerState.pickingMode === 'return') {
                    pickerState.returnDate = new Date(pickerState.calendarYear, pickerState.calendarMonth, day);
                    triggerHaptic('success', `Return set: ${day} ${monthNames[pickerState.calendarMonth]}`);
                } else {
...
function triggerHomepageCompanion() {
    ...
    if (recentTitle) {
        recentTitle.innerText = 'CAB DEALS';
        recentTitle.style.textTransform = 'uppercase';
    }
    ...
    if (recentGrid) {
        ...
        recentGrid.innerHTML = `
            <div style="display: flex; align-items: center; background: rgba(255, 255, 255, 0.85); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); border: 1px solid rgba(255,255,255,0.9); border-radius: 16px; padding: 12px 16px; box-shadow: 0 8px 24px rgba(0,31,84,0.08); position: relative; overflow: hidden; width: 100%; cursor: pointer; min-height: 80px;">
                ...
                <img src="https://pngimg.com/uploads/taxi/taxi_PNG74.png" alt="Taxi" style="position: absolute; right: 5px; bottom: 2px; width: 120px; z-index: 1; filter: drop-shadow(0 6px 12px rgba(0,0,0,0.25));" />
                
                <div style="position: absolute; right: 12px; top: 12px; z-index: 3; background: #fff; border-radius: 50%; width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                    <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="#0f172a" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
                </div>

                <!-- Animated Keychain SVG coming from left -->
                <div style="position: absolute; left: -100px; top: -10px; width: 60px; height: 120px; animation: slideInKeychain 0.6s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards; z-index: 2;">
                    <div style="width: 100%; height: 100%; animation: swingKeychain 2.5s ease-in-out infinite alternate; transform-origin: top center;">
                        <!-- Simple SVG representation of the G keychain -->
                        <svg viewBox="0 0 100 200" width="100%" height="100%" style="filter: drop-shadow(4px 10px 12px rgba(0,0,0,0.2));">
                            <!-- Clip -->
                ...
                <div style="flex: 1; padding-left: 70px; padding-right: 40px; z-index: 3; position: relative;">
                    <div style="font-size: 16px; font-weight: 800; color: #0f172a; margin-bottom: 6px; letter-spacing: -0.2px;">Airport to City</div>
                    <div style="display: inline-block; background: rgba(16, 185, 129, 0.1); color: #10b981; padding: 4px 10px; border-radius: 12px; font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.5px; border: 1px solid rgba(16, 185, 129, 0.2); box-shadow: 0 2px 4px rgba(16, 185, 129, 0.1);">20% OFF</div>
                </div>
            </div>
    ...
    // Inject Hotel Deals Carousel
    let hotelSection = document.getElementById('companionHotelDeals');
    if (!hotelSection) {
        hotelSection = document.createElement('div');
        hotelSection.id = 'companionHotelDeals';
        hotelSection.style.marginTop = '32px';
        hotelSection.style.order = '5'; // Ensure it appears after recentSearchesSection (which has order 4)
        
        hotelSection.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">
                <div style="display: flex; align-items: center; gap: 10px;">
                    <span class="section-title">HOTEL DEALS</span>
                    <!-- Torn Ticket Coupon -->
            ...
            <div style="display: flex; gap: 14px; overflow-x: auto; padding-bottom: 16px; margin-left: -16px; padding-left: 16px; padding-right: 16px; scroll-snap-type: x mandatory; -webkit-overflow-scrolling: touch; scrollbar-width: none;">
                
                <!-- CRED-Inspired Hotel Card 1 -->
                <div style="flex: 0 0 145px; height: 195px; scroll-snap-align: center; border-radius: 18px; overflow: hidden; position: relative; box-shadow: 0 12px 30px rgba(0,0,0,0.15); border: 1px solid rgba(255,255,255,0.4); cursor: pointer;">
                    ...
                    <!-- Neon Discount Badge -->
                    <div style="position: absolute; top: 12px; left: 12px; background: rgba(16, 185, 129, 0.95); color: #fff; padding: 4px 10px; border-radius: 8px; font-size: 10px; font-weight: 900; text-transform: uppercase; z-index: 3; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.5); letter-spacing: 0.5px;">
                        30% OFF
                    </div>
                    
                    <!-- Content -->
                    <div style="position: absolute; bottom: 0; left: 0; width: 100%; padding: 14px; z-index: 3; box-sizing: border-box;">
                        <h4 style="margin: 0 0 2px 0; font-size: 16px; font-weight: 800; color: #fff; line-height: 1.2;">Taj Lands End</h4>
                        <div style="font-size: 11px; font-weight: 600; color: #94a3b8; margin-bottom: 10px;">Mumbai</div>
                        
                        <div style="display: flex; align-items: flex-end; justify-content: space-between;">
                            <div>
                                <div style="font-size: 10px; color: #cbd5e1; text-decoration: line-through; opacity: 0.7;">₹18,500</div>
                                <div style="font-size: 18px; font-weight: 900; color: #fff; letter-spacing: -0.5px;">₹12,950</div>
                            </div>
                            <!-- Frosted Arrow Button -->
                            <div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); padding: 8px; border-radius: 50%; color: #fff; border: 1px solid rgba(255,255,255,0.25);">
                                <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div style="flex: 0 0 170px; height: 230px; scroll-snap-align: center; border-radius: 18px; overflow: hidden; position: relative; box-shadow: 0 12px 30px rgba(0,0,0,0.15); border: 1px solid rgba(255,255,255,0.4); cursor: pointer;">
                    ...
                    <div style="position: absolute; bottom: 0; left: 0; width: 100%; padding: 14px; z-index: 3; box-sizing: border-box;">
                        <h4 style="margin: 0 0 2px 0; font-size: 16px; font-weight: 800; color: #fff; line-height: 1.2;">Trident Hotel</h4>
                        <div style="font-size: 11px; font-weight: 600; color: #94a3b8; margin-bottom: 10px;">Nariman Point</div>
                        
                        <div style="display: flex; align-items: flex-end; justify-content: space-between;">
                            <div>
                                <div style="font-size: 10px; color: #cbd5e1; text-decoration: line-through; opacity: 0.7;">₹15,000</div>
                                <div style="font-size: 18px; font-weight: 900; color: #fff; letter-spacing: -0.5px;">₹11,250</div>
                            </div>
                            <div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); padding: 8px; border-radius: 50%; color: #fff; border: 1px solid rgba(255,255,255,0.25);">
                                <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
                            </div>
                ...
                <div style="flex: 0 0 170px; height: 230px; scroll-snap-align: center; border-radius: 18px; overflow: hidden; position: relative; box-shadow: 0 12px 30px rgba(0,0,0,0.15); border: 1px solid rgba(255,255,255,0.4); cursor: pointer;">
                    ...
                    <div style="position: absolute; top: 12px; left: 12px; background: rgba(59, 130, 246, 0.95); color: #fff; padding: 4px 10px; border-radius: 8px; font-size: 10px; font-weight: 900; text-transform: uppercase; z-index: 3; box-shadow: 0 4px 12px rgba(59, 130, 246, 0.5); letter-spacing: 0.5px;">
                        NEW
                    </div>
                    
                    <div style="position: absolute; bottom: 0; left: 0; width: 100%; padding: 14px; z-index: 3; box-sizing: border-box;">
                        <h4 style="margin: 0 0 2px 0; font-size: 16px; font-weight: 800; color: #fff; line-height: 1.2;">JW Marriott</h4>
                        <div style="font-size: 11px; font-weight: 600; color: #94a3b8; margin-bottom: 10px;">Juhu</div>
                        
                        <div style="display: flex; align-items: flex-end; justify-content: space-between;">
                            <div>
                                <div style="font-size: 10px; color: #cbd5e1; text-decoration: line-through; opacity: 0.7;">₹21,000</div>
                                <div style="font-size: 18px; font-weight: 900; color: #fff; letter-spacing: -0.5px;">₹18,500</div>
                            </div>
                            <div style="background: rgba(255,255,255,0.15); backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); padding: 8px; border-radius: 50%; color: #fff; border: 1px solid rgba(255,255,255,0.25);">
                                <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
                            </div>
            ...
            <style>
                #companionHotelDeals ::-webkit-scrollbar { display: none; }
            </style>
        `;
        
        if (recentSection && recentSection.parentNode) {
            recentSection.parentNode.insertBefore(hotelSection, recentSection.nextSibling);
        }
    } else {
