...
start_tag = '<div class="recent-searches-section" id="recentSearchesSection">'
start_idx = content.find(start_tag)

if start_idx != -1:
    end_home_idx = content.find('SCREEN 2: OFFERS & DEALS', start_idx)
    if end_home_idx != -1:
        # Find the last </div> before end_home_idx
        last_div_idx = content.rfind('</div>', start_idx, end_home_idx)
        if last_div_idx != -1:
            new_content = content[:last_div_idx] + cab_deals + hotel_deals + "\n" + content[last_div_idx:]
            with open('index.html', 'w') as f:
                f.write(new_content)
            print("Successfully injected at the end of screenHome!")
        else:
            print("Could not find closing div")
    else:
        print("Could not find SCREEN 2 marker")
else:
    print("Could not find recentSearchesSection")
